...
! =========  ExtInfw_InitInputType_C  =======
  TYPE, BIND(C) :: ExtInfw_InitInputType_C
   TYPE(C_PTR) :: object = C_NULL_PTR
    ...
    TYPE(C_ptr) :: nStructBldEtaNodes = C_NULL_PTR 
    INTEGER(C_int) :: nStructBldEtaNodes_Len = 0 
    TYPE(C_ptr) :: StructBldEtaNodes = C_NULL_PTR 
    INTEGER(C_int) :: StructBldEtaNodes_Len = 0 
    TYPE(C_ptr) :: StructTwrEtaNodes = C_NULL_PTR 
    INTEGER(C_int) :: StructTwrEtaNodes_Len = 0 
  ...
  TYPE, PUBLIC :: ExtInfw_InitInputType
    ...
    INTEGER(KIND=C_INT) , DIMENSION(:), POINTER  :: nStructBldEtaNodes => NULL()      !< Number of structural model nodes for each blade [-]
    REAL(KIND=C_DOUBLE) , DIMENSION(:), POINTER  :: StructBldEtaNodes => NULL()      !< Location of blade structural model nodes in local coordinates [0-1]. The arrays for each blade are concatenated. [-]
    REAL(KIND=C_DOUBLE) , DIMENSION(:), POINTER  :: StructTwrEtaNodes => NULL()      !< Location of tower structural model nodes in local coodinates [0-1] [-]
...
! =========  ExtInfw_MiscVarType_C  =======
  ...
  TYPE, PUBLIC :: ExtInfw_MiscVarType
    ...
    TYPE(MeshType) , DIMENSION(:), ALLOCATABLE  :: ActForceLoads      !< line2 mesh for transferring AeroDyn distributed loads to ExternalInflow [-]
    TYPE(MeshType) , DIMENSION(:), ALLOCATABLE  :: ActForceMotions      !< line2 mesh for transferring AeroDyn distributed loads to ExternalInflow (needs translationDisp) [-]
    TYPE(MeshType) , DIMENSION(:), ALLOCATABLE  :: ActForceMotionsPoints      !< point mesh for transferring AeroDyn distributed loads to ExternalInflow (needs translationDisp) [-]
    TYPE(MeshType) , DIMENSION(:), ALLOCATABLE  :: ActForceLoadsPoints      !< point mesh for transferring AeroDyn distributed loads to ExternalInflow [-]
...
! =========  ExtInfw_ParameterType_C  =======
  TYPE, BIND(C) :: ExtInfw_ParameterType_C
   TYPE(C_PTR) :: object = C_NULL_PTR
    ...
    TYPE(C_ptr) :: forceBldEtaNodes = C_NULL_PTR 
    INTEGER(C_int) :: forceBldEtaNodes_Len = 0 
    TYPE(C_ptr) :: forceTwrEtaNodes = C_NULL_PTR 
    INTEGER(C_int) :: forceTwrEtaNodes_Len = 0 
  END TYPE ExtInfw_ParameterType_C
  TYPE, PUBLIC :: ExtInfw_ParameterType
    TYPE( ExtInfw_ParameterType_C ) :: C_obj
    REAL(ReKi)  :: AirDens      !< Air density for normalization of loads sent to ExternalInflow [kg/m^3]
    INTEGER(IntKi)  :: NumBl      !< Number of blades [-]
    INTEGER(IntKi)  :: NMappings      !< Number of mappings [-]
    INTEGER(IntKi)  :: NnodesVel      !< number of velocity nodes on OpenFAST-CFD interface [-]
    INTEGER(IntKi)  :: NnodesForce      !< number of force nodes on OpenFAST-CFD interface [-]
    INTEGER(IntKi)  :: NnodesForceBlade      !< number of force nodes on OpenFAST-CFD interface [-]
    INTEGER(IntKi)  :: NnodesForceTower      !< number of force nodes on OpemFAST-CFD interface [-]
    REAL(KIND=C_FLOAT) , DIMENSION(:), POINTER  :: forceBldEtaNodes => NULL() 
    REAL(KIND=C_FLOAT) , DIMENSION(:), POINTER  :: forceTwrEtaNodes => NULL() 
...
! =========  ExtInfw_InputType_C  =======
  TYPE, BIND(C) :: ExtInfw_InputType_C
   TYPE(C_PTR) :: object = C_NULL_PTR
    ...
    TYPE(C_ptr) :: pxdotVel = C_NULL_PTR 
    INTEGER(C_int) :: pxdotVel_Len = 0 
    TYPE(C_ptr) :: pydotVel = C_NULL_PTR 
    INTEGER(C_int) :: pydotVel_Len = 0 
    TYPE(C_ptr) :: pzdotVel = C_NULL_PTR 
    INTEGER(C_int) :: pzdotVel_Len = 0 
    ...
    TYPE(C_ptr) :: pxdotForce = C_NULL_PTR 
    INTEGER(C_int) :: pxdotForce_Len = 0 
    TYPE(C_ptr) :: pydotForce = C_NULL_PTR 
    INTEGER(C_int) :: pydotForce_Len = 0 
    TYPE(C_ptr) :: pzdotForce = C_NULL_PTR 
    INTEGER(C_int) :: pzdotForce_Len = 0 
  ...
  TYPE, PUBLIC :: ExtInfw_InputType
    ...
    REAL(KIND=C_FLOAT) , DIMENSION(:), POINTER  :: pxdotVel => NULL()      !< x of velocity interface (Aerodyn) nodes [m]
    REAL(KIND=C_FLOAT) , DIMENSION(:), POINTER  :: pydotVel => NULL()      !< y position of velocity interface (Aerodyn) nodes [m]
    REAL(KIND=C_FLOAT) , DIMENSION(:), POINTER  :: pzdotVel => NULL()      !< z position of velocity interface (Aerodyn) nodes [m]
    ...
    REAL(KIND=C_FLOAT) , DIMENSION(:), POINTER  :: pxdotForce => NULL()      !< x velocity of actuator force nodes [m/s]
    REAL(KIND=C_FLOAT) , DIMENSION(:), POINTER  :: pydotForce => NULL()      !< y velocity of actuator force nodes [m/s]
    REAL(KIND=C_FLOAT) , DIMENSION(:), POINTER  :: pzdotForce => NULL()      !< z velocity of actuator force nodes [m/s]
...
IF (ASSOCIATED(SrcInitInputData%nStructBldEtaNodes)) THEN
  i1_l = LBOUND(SrcInitInputData%nStructBldEtaNodes,1)
  i1_u = UBOUND(SrcInitInputData%nStructBldEtaNodes,1)
  IF (.NOT. ASSOCIATED(DstInitInputData%nStructBldEtaNodes)) THEN 
    ALLOCATE(DstInitInputData%nStructBldEtaNodes(i1_l:i1_u),STAT=ErrStat2)
    IF (ErrStat2 /= 0) THEN 
      CALL SetErrStat(ErrID_Fatal, 'Error allocating DstInitInputData%nStructBldEtaNodes.', ErrStat, ErrMsg,RoutineName)
      RETURN
    END IF
    DstInitInputData%c_obj%nStructBldEtaNodes_Len = SIZE(DstInitInputData%nStructBldEtaNodes)
    IF (DstInitInputData%c_obj%nStructBldEtaNodes_Len > 0) &
          DstInitInputData%c_obj%nStructBldEtaNodes = C_LOC( DstInitInputData%nStructBldEtaNodes( i1_l ) )
  END IF
    DstInitInputData%nStructBldEtaNodes = SrcInitInputData%nStructBldEtaNodes
ENDIF
IF (ASSOCIATED(SrcInitInputData%StructBldEtaNodes)) THEN
  i1_l = LBOUND(SrcInitInputData%StructBldEtaNodes,1)
  i1_u = UBOUND(SrcInitInputData%StructBldEtaNodes,1)
  IF (.NOT. ASSOCIATED(DstInitInputData%StructBldEtaNodes)) THEN 
    ALLOCATE(DstInitInputData%StructBldEtaNodes(i1_l:i1_u),STAT=ErrStat2)
    IF (ErrStat2 /= 0) THEN 
      CALL SetErrStat(ErrID_Fatal, 'Error allocating DstInitInputData%StructBldEtaNodes.', ErrStat, ErrMsg,RoutineName)
      RETURN
    END IF
    DstInitInputData%c_obj%StructBldEtaNodes_Len = SIZE(DstInitInputData%StructBldEtaNodes)
    IF (DstInitInputData%c_obj%StructBldEtaNodes_Len > 0) &
          DstInitInputData%c_obj%StructBldEtaNodes = C_LOC( DstInitInputData%StructBldEtaNodes( i1_l ) )
  END IF
    DstInitInputData%StructBldEtaNodes = SrcInitInputData%StructBldEtaNodes
ENDIF
IF (ASSOCIATED(SrcInitInputData%StructTwrEtaNodes)) THEN
  i1_l = LBOUND(SrcInitInputData%StructTwrEtaNodes,1)
  i1_u = UBOUND(SrcInitInputData%StructTwrEtaNodes,1)
  IF (.NOT. ASSOCIATED(DstInitInputData%StructTwrEtaNodes)) THEN 
    ALLOCATE(DstInitInputData%StructTwrEtaNodes(i1_l:i1_u),STAT=ErrStat2)
    IF (ErrStat2 /= 0) THEN 
      CALL SetErrStat(ErrID_Fatal, 'Error allocating DstInitInputData%StructTwrEtaNodes.', ErrStat, ErrMsg,RoutineName)
      RETURN
    END IF
    DstInitInputData%c_obj%StructTwrEtaNodes_Len = SIZE(DstInitInputData%StructTwrEtaNodes)
    IF (DstInitInputData%c_obj%StructTwrEtaNodes_Len > 0) &
          DstInitInputData%c_obj%StructTwrEtaNodes = C_LOC( DstInitInputData%StructTwrEtaNodes( i1_l ) )
  END IF
    DstInitInputData%StructTwrEtaNodes = SrcInitInputData%StructTwrEtaNodes
ENDIF
    DstInitInputData%NumBl = SrcInitInputData%NumBl
    DstInitInputData%C_obj%NumBl = SrcInitInputData%C_obj%NumBl
 ...
 END SUBROUTINE ExtInfw_CopyInitInput

 SUBROUTINE ExtInfw_DestroyInitInput( InitInputData, ErrStat, ErrMsg, DEALLOCATEpointers )
...
IF (ASSOCIATED(InitInputData%nStructBldEtaNodes)) THEN
 IF (DEALLOCATEpointers_local) &
  DEALLOCATE(InitInputData%nStructBldEtaNodes)
  InitInputData%nStructBldEtaNodes => NULL()
  InitInputData%C_obj%nStructBldEtaNodes = C_NULL_PTR
  InitInputData%C_obj%nStructBldEtaNodes_Len = 0
ENDIF
IF (ASSOCIATED(InitInputData%StructBldEtaNodes)) THEN
 IF (DEALLOCATEpointers_local) &
  DEALLOCATE(InitInputData%StructBldEtaNodes)
  InitInputData%StructBldEtaNodes => NULL()
  InitInputData%C_obj%StructBldEtaNodes = C_NULL_PTR
  InitInputData%C_obj%StructBldEtaNodes_Len = 0
ENDIF
IF (ASSOCIATED(InitInputData%StructTwrEtaNodes)) THEN
 IF (DEALLOCATEpointers_local) &
  DEALLOCATE(InitInputData%StructTwrEtaNodes)
  InitInputData%StructTwrEtaNodes => NULL()
  InitInputData%C_obj%StructTwrEtaNodes = C_NULL_PTR
  InitInputData%C_obj%StructTwrEtaNodes_Len = 0
ENDIF
 END SUBROUTINE ExtInfw_DestroyInitInput

 SUBROUTINE ExtInfw_PackInitInput( ReKiBuf, DbKiBuf, IntKiBuf, Indata, ErrStat, ErrMsg, SizeOnly )
 ...
 ! buffers to store subtypes, if any
  ...
  Int_BufSz   = Int_BufSz   + 1     ! nStructBldEtaNodes allocated yes/no
  IF ( ASSOCIATED(InData%nStructBldEtaNodes) ) THEN
    Int_BufSz   = Int_BufSz   + 2*1  ! nStructBldEtaNodes upper/lower bounds for each dimension
      Int_BufSz  = Int_BufSz  + SIZE(InData%nStructBldEtaNodes)  ! nStructBldEtaNodes
  END IF
  Int_BufSz   = Int_BufSz   + 1     ! StructBldEtaNodes allocated yes/no
  IF ( ASSOCIATED(InData%StructBldEtaNodes) ) THEN
    Int_BufSz   = Int_BufSz   + 2*1  ! StructBldEtaNodes upper/lower bounds for each dimension
      Db_BufSz   = Db_BufSz   + SIZE(InData%StructBldEtaNodes)  ! StructBldEtaNodes
  END IF
  Int_BufSz   = Int_BufSz   + 1     ! StructTwrEtaNodes allocated yes/no
  IF ( ASSOCIATED(InData%StructTwrEtaNodes) ) THEN
    Int_BufSz   = Int_BufSz   + 2*1  ! StructTwrEtaNodes upper/lower bounds for each dimension
      Db_BufSz   = Db_BufSz   + SIZE(InData%StructTwrEtaNodes)  ! StructTwrEtaNodes
  END IF
  ...
  Int_Xferred = 1
    ...
    Int_Xferred = Int_Xferred + 1
  IF ( .NOT. ASSOCIATED(InData%nStructBldEtaNodes) ) THEN
    IntKiBuf( Int_Xferred ) = 0
    Int_Xferred = Int_Xferred + 1
  ELSE
    IntKiBuf( Int_Xferred ) = 1
    Int_Xferred = Int_Xferred + 1
    IntKiBuf( Int_Xferred    ) = LBOUND(InData%nStructBldEtaNodes,1)
    IntKiBuf( Int_Xferred + 1) = UBOUND(InData%nStructBldEtaNodes,1)
    Int_Xferred = Int_Xferred + 2

      DO i1 = LBOUND(InData%nStructBldEtaNodes,1), UBOUND(InData%nStructBldEtaNodes,1)
        IntKiBuf(Int_Xferred) = InData%nStructBldEtaNodes(i1)
        Int_Xferred = Int_Xferred + 1
      END DO
  END IF
  IF ( .NOT. ASSOCIATED(InData%StructBldEtaNodes) ) THEN
    IntKiBuf( Int_Xferred ) = 0
    Int_Xferred = Int_Xferred + 1
  ELSE
    IntKiBuf( Int_Xferred ) = 1
    Int_Xferred = Int_Xferred + 1
    IntKiBuf( Int_Xferred    ) = LBOUND(InData%StructBldEtaNodes,1)
    IntKiBuf( Int_Xferred + 1) = UBOUND(InData%StructBldEtaNodes,1)
    Int_Xferred = Int_Xferred + 2

      DO i1 = LBOUND(InData%StructBldEtaNodes,1), UBOUND(InData%StructBldEtaNodes,1)
        DbKiBuf(Db_Xferred) = InData%StructBldEtaNodes(i1)
        Db_Xferred = Db_Xferred + 1
      END DO
  END IF
  IF ( .NOT. ASSOCIATED(InData%StructTwrEtaNodes) ) THEN
    IntKiBuf( Int_Xferred ) = 0
    Int_Xferred = Int_Xferred + 1
  ELSE
    IntKiBuf( Int_Xferred ) = 1
    Int_Xferred = Int_Xferred + 1
    IntKiBuf( Int_Xferred    ) = LBOUND(InData%StructTwrEtaNodes,1)
    IntKiBuf( Int_Xferred + 1) = UBOUND(InData%StructTwrEtaNodes,1)
    Int_Xferred = Int_Xferred + 2

      DO i1 = LBOUND(InData%StructTwrEtaNodes,1), UBOUND(InData%StructTwrEtaNodes,1)
        DbKiBuf(Db_Xferred) = InData%StructTwrEtaNodes(i1)
        Db_Xferred = Db_Xferred + 1
      END DO
  END IF
    ...
    Re_Xferred = Re_Xferred + 1
 END SUBROUTINE ExtInfw_PackInitInput

 SUBROUTINE ExtInfw_UnPackInitInput( ReKiBuf, DbKiBuf, IntKiBuf, Outdata, ErrStat, ErrMsg )
 ...
 ! buffers to store meshes, if any
  ...
  IF ( IntKiBuf( Int_Xferred ) == 0 ) THEN  ! nStructBldEtaNodes not allocated
    Int_Xferred = Int_Xferred + 1
  ELSE
    Int_Xferred = Int_Xferred + 1
    ...
    Int_Xferred = Int_Xferred + 2
    IF (ASSOCIATED(OutData%nStructBldEtaNodes)) DEALLOCATE(OutData%nStructBldEtaNodes)
    ALLOCATE(OutData%nStructBldEtaNodes(i1_l:i1_u),STAT=ErrStat2)
    IF (ErrStat2 /= 0) THEN 
       CALL SetErrStat(ErrID_Fatal, 'Error allocating OutData%nStructBldEtaNodes.', ErrStat, ErrMsg,RoutineName)
       RETURN
    END IF
    OutData%c_obj%nStructBldEtaNodes_Len = SIZE(OutData%nStructBldEtaNodes)
    IF (OutData%c_obj%nStructBldEtaNodes_Len > 0) &
       OutData%c_obj%nStructBldEtaNodes = C_LOC( OutData%nStructBldEtaNodes( i1_l ) )
      DO i1 = LBOUND(OutData%nStructBldEtaNodes,1), UBOUND(OutData%nStructBldEtaNodes,1)
        OutData%nStructBldEtaNodes(i1) = IntKiBuf(Int_Xferred)
        Int_Xferred = Int_Xferred + 1
      END DO
  END IF
  IF ( IntKiBuf( Int_Xferred ) == 0 ) THEN  ! StructBldEtaNodes not allocated
    Int_Xferred = Int_Xferred + 1
  ELSE
    Int_Xferred = Int_Xferred + 1
    i1_l = IntKiBuf( Int_Xferred    )
    i1_u = IntKiBuf( Int_Xferred + 1)
    Int_Xferred = Int_Xferred + 2
    IF (ASSOCIATED(OutData%StructBldEtaNodes)) DEALLOCATE(OutData%StructBldEtaNodes)
    ALLOCATE(OutData%StructBldEtaNodes(i1_l:i1_u),STAT=ErrStat2)
    IF (ErrStat2 /= 0) THEN 
       CALL SetErrStat(ErrID_Fatal, 'Error allocating OutData%StructBldEtaNodes.', ErrStat, ErrMsg,RoutineName)
       RETURN
    END IF
    OutData%c_obj%StructBldEtaNodes_Len = SIZE(OutData%StructBldEtaNodes)
    IF (OutData%c_obj%StructBldEtaNodes_Len > 0) &
       OutData%c_obj%StructBldEtaNodes = C_LOC( OutData%StructBldEtaNodes( i1_l ) )
      DO i1 = LBOUND(OutData%StructBldEtaNodes,1), UBOUND(OutData%StructBldEtaNodes,1)
        OutData%StructBldEtaNodes(i1) = REAL(DbKiBuf(Db_Xferred), C_DOUBLE)
        Db_Xferred = Db_Xferred + 1
      END DO
  END IF
  IF ( IntKiBuf( Int_Xferred ) == 0 ) THEN  ! StructTwrEtaNodes not allocated
    Int_Xferred = Int_Xferred + 1
  ELSE
    Int_Xferred = Int_Xferred + 1
    i1_l = IntKiBuf( Int_Xferred    )
    i1_u = IntKiBuf( Int_Xferred + 1)
    Int_Xferred = Int_Xferred + 2
    IF (ASSOCIATED(OutData%StructTwrEtaNodes)) DEALLOCATE(OutData%StructTwrEtaNodes)
    ALLOCATE(OutData%StructTwrEtaNodes(i1_l:i1_u),STAT=ErrStat2)
    IF (ErrStat2 /= 0) THEN 
       CALL SetErrStat(ErrID_Fatal, 'Error allocating OutData%StructTwrEtaNodes.', ErrStat, ErrMsg,RoutineName)
       RETURN
    END IF
    OutData%c_obj%StructTwrEtaNodes_Len = SIZE(OutData%StructTwrEtaNodes)
    IF (OutData%c_obj%StructTwrEtaNodes_Len > 0) &
       OutData%c_obj%StructTwrEtaNodes = C_LOC( OutData%StructTwrEtaNodes( i1_l ) )
      DO i1 = LBOUND(OutData%StructTwrEtaNodes,1), UBOUND(OutData%StructTwrEtaNodes,1)
        OutData%StructTwrEtaNodes(i1) = REAL(DbKiBuf(Db_Xferred), C_DOUBLE)
        Db_Xferred = Db_Xferred + 1
      END DO
  END IF
 ...
 END SUBROUTINE ExtInfw_UnPackInitInput
 ...
 SUBROUTINE ExtInfw_C2Fary_CopyInitInput( InitInputData, ErrStat, ErrMsg, SkipPointers )
    ...
    ! -- nStructBldEtaNodes InitInput Data fields
    IF ( .NOT. SkipPointers_local ) THEN
       IF ( .NOT. C_ASSOCIATED( InitInputData%C_obj%nStructBldEtaNodes ) ) THEN
          NULLIFY( InitInputData%nStructBldEtaNodes )
       ELSE
          CALL C_F_POINTER(InitInputData%C_obj%nStructBldEtaNodes, InitInputData%nStructBldEtaNodes, (/InitInputData%C_obj%nStructBldEtaNodes_Len/))
       END IF
    END IF

    ! -- StructBldEtaNodes InitInput Data fields
    IF ( .NOT. SkipPointers_local ) THEN
       IF ( .NOT. C_ASSOCIATED( InitInputData%C_obj%StructBldEtaNodes ) ) THEN
          NULLIFY( InitInputData%StructBldEtaNodes )
       ELSE
          CALL C_F_POINTER(InitInputData%C_obj%StructBldEtaNodes, InitInputData%StructBldEtaNodes, (/InitInputData%C_obj%StructBldEtaNodes_Len/))
       END IF
    END IF

    ! -- StructTwrEtaNodes InitInput Data fields
    IF ( .NOT. SkipPointers_local ) THEN
       IF ( .NOT. C_ASSOCIATED( InitInputData%C_obj%StructTwrEtaNodes ) ) THEN
          NULLIFY( InitInputData%StructTwrEtaNodes )
       ELSE
          CALL C_F_POINTER(InitInputData%C_obj%StructTwrEtaNodes, InitInputData%StructTwrEtaNodes, (/InitInputData%C_obj%StructTwrEtaNodes_Len/))
       END IF
    END IF
    InitInputData%NumBl = InitInputData%C_obj%NumBl
 ...
 END SUBROUTINE ExtInfw_C2Fary_CopyInitInput
 ...
 SUBROUTINE ExtInfw_F2C_CopyInitInput( InitInputData, ErrStat, ErrMsg, SkipPointers  )
    ...
    ! -- nStructBldEtaNodes InitInput Data fields
    IF ( .NOT. SkipPointers_local ) THEN
       IF ( .NOT. ASSOCIATED(InitInputData%nStructBldEtaNodes)) THEN 
          InitInputData%c_obj%nStructBldEtaNodes_Len = 0
          InitInputData%c_obj%nStructBldEtaNodes = C_NULL_PTR
       ELSE
          InitInputData%c_obj%nStructBldEtaNodes_Len = SIZE(InitInputData%nStructBldEtaNodes)
          IF (InitInputData%c_obj%nStructBldEtaNodes_Len > 0) &
             InitInputData%c_obj%nStructBldEtaNodes = C_LOC( InitInputData%nStructBldEtaNodes( LBOUND(InitInputData%nStructBldEtaNodes,1) ) )
       END IF
    END IF

    ! -- StructBldEtaNodes InitInput Data fields
    IF ( .NOT. SkipPointers_local ) THEN
       IF ( .NOT. ASSOCIATED(InitInputData%StructBldEtaNodes)) THEN 
          InitInputData%c_obj%StructBldEtaNodes_Len = 0
          InitInputData%c_obj%StructBldEtaNodes = C_NULL_PTR
       ELSE
          InitInputData%c_obj%StructBldEtaNodes_Len = SIZE(InitInputData%StructBldEtaNodes)
          IF (InitInputData%c_obj%StructBldEtaNodes_Len > 0) &
             InitInputData%c_obj%StructBldEtaNodes = C_LOC( InitInputData%StructBldEtaNodes( LBOUND(InitInputData%StructBldEtaNodes,1) ) )
       END IF
    END IF

    ! -- StructTwrEtaNodes InitInput Data fields
    IF ( .NOT. SkipPointers_local ) THEN
       IF ( .NOT. ASSOCIATED(InitInputData%StructTwrEtaNodes)) THEN 
          InitInputData%c_obj%StructTwrEtaNodes_Len = 0
          InitInputData%c_obj%StructTwrEtaNodes = C_NULL_PTR
       ELSE
          InitInputData%c_obj%StructTwrEtaNodes_Len = SIZE(InitInputData%StructTwrEtaNodes)
          IF (InitInputData%c_obj%StructTwrEtaNodes_Len > 0) &
             InitInputData%c_obj%StructTwrEtaNodes = C_LOC( InitInputData%StructTwrEtaNodes( LBOUND(InitInputData%StructTwrEtaNodes,1) ) )
       END IF
    END IF
    InitInputData%C_obj%NumBl = InitInputData%NumBl
 ...
 END SUBROUTINE ExtInfw_F2C_CopyInitInput
...
ENDIF
 ...
 END SUBROUTINE ExtInfw_CopyInitOutput

 SUBROUTINE ExtInfw_DestroyInitOutput( InitOutputData, ErrStat, ErrMsg, DEALLOCATEpointers )
...
ENDIF
  CALL NWTC_Library_Destroyprogdesc( InitOutputData%Ver, ErrStat2, ErrMsg2, DEALLOCATEpointers_local )
     CALL SetErrStat(ErrStat2, ErrMsg2, ErrStat, ErrMsg, RoutineName)
 END SUBROUTINE ExtInfw_DestroyInitOutput

 SUBROUTINE ExtInfw_PackInitOutput( ReKiBuf, DbKiBuf, IntKiBuf, Indata, ErrStat, ErrMsg, SizeOnly )
  REAL(ReKi),       ALLOCATABLE, INTENT(  OUT) :: ReKiBuf(:)
  REAL(DbKi),       ALLOCATABLE, INTENT(  OUT) :: DbKiBuf(:)
  INTEGER(IntKi),   ALLOCATABLE, INTENT(  OUT) :: IntKiBuf(:)
 ...
 ! buffers to store subtypes, if any
  ...
  END IF
      ...
      ELSE
        IntKiBuf( Int_Xferred ) = 0; Int_Xferred = Int_Xferred + 1
      ENDIF
 END SUBROUTINE ExtInfw_PackInitOutput

 SUBROUTINE ExtInfw_UnPackInitOutput( ReKiBuf, DbKiBuf, IntKiBuf, Outdata, ErrStat, ErrMsg )
  REAL(ReKi),      ALLOCATABLE, INTENT(IN   ) :: ReKiBuf(:)
  REAL(DbKi),      ALLOCATABLE, INTENT(IN   ) :: DbKiBuf(:)
  INTEGER(IntKi),  ALLOCATABLE, INTENT(IN   ) :: IntKiBuf(:)
 ...
 END SUBROUTINE ExtInfw_UnPackInitOutput
 ...
 SUBROUTINE ExtInfw_C2Fary_CopyInitOutput( InitOutputData, ErrStat, ErrMsg, SkipPointers )
    ...
    ELSE
       SkipPointers_local = .false.
    END IF
 END SUBROUTINE ExtInfw_C2Fary_CopyInitOutput
 ...
 SUBROUTINE ExtInfw_F2C_CopyInitOutput( InitOutputData, ErrStat, ErrMsg, SkipPointers  )
    ...
    ELSE
       SkipPointers_local = .false.
    END IF
 END SUBROUTINE ExtInfw_F2C_CopyInitOutput
...
IF (ALLOCATED(SrcMiscData%Line2_to_Point_Motions)) THEN
  ...
  END IF
    ...
    ENDDO
ENDIF
 END SUBROUTINE ExtInfw_CopyMisc

 SUBROUTINE ExtInfw_DestroyMisc( MiscData, ErrStat, ErrMsg, DEALLOCATEpointers )
...
ENDIF
 END SUBROUTINE ExtInfw_DestroyMisc

 SUBROUTINE ExtInfw_PackMisc( ReKiBuf, DbKiBuf, IntKiBuf, Indata, ErrStat, ErrMsg, SizeOnly )
  REAL(ReKi),       ALLOCATABLE, INTENT(  OUT) :: ReKiBuf(:)
  REAL(DbKi),       ALLOCATABLE, INTENT(  OUT) :: DbKiBuf(:)
  INTEGER(IntKi),   ALLOCATABLE, INTENT(  OUT) :: IntKiBuf(:)
 ...
 ! buffers to store subtypes, if any
  ...
  ELSE
    ...
    END DO
  END IF
 END SUBROUTINE ExtInfw_PackMisc

 SUBROUTINE ExtInfw_UnPackMisc( ReKiBuf, DbKiBuf, IntKiBuf, Outdata, ErrStat, ErrMsg )
  REAL(ReKi),      ALLOCATABLE, INTENT(IN   ) :: ReKiBuf(:)
  REAL(DbKi),      ALLOCATABLE, INTENT(IN   ) :: DbKiBuf(:)
  INTEGER(IntKi),  ALLOCATABLE, INTENT(IN   ) :: IntKiBuf(:)
 ...
 ! buffers to store meshes, if any
  ...
  ELSE
    ...
    END DO
  END IF
 END SUBROUTINE ExtInfw_UnPackMisc
 ...
 SUBROUTINE ExtInfw_C2Fary_CopyMisc( MiscData, ErrStat, ErrMsg, SkipPointers )
    ...
    ELSE
       SkipPointers_local = .false.
    END IF
 END SUBROUTINE ExtInfw_C2Fary_CopyMisc
 ...
 SUBROUTINE ExtInfw_F2C_CopyMisc( MiscData, ErrStat, ErrMsg, SkipPointers  )
    ...
    ELSE
       SkipPointers_local = .false.
    END IF
 END SUBROUTINE ExtInfw_F2C_CopyMisc
...
IF (ASSOCIATED(SrcParamData%forceBldEtaNodes)) THEN
  i1_l = LBOUND(SrcParamData%forceBldEtaNodes,1)
  i1_u = UBOUND(SrcParamData%forceBldEtaNodes,1)
  IF (.NOT. ASSOCIATED(DstParamData%forceBldEtaNodes)) THEN 
    ALLOCATE(DstParamData%forceBldEtaNodes(i1_l:i1_u),STAT=ErrStat2)
    IF (ErrStat2 /= 0) THEN 
      CALL SetErrStat(ErrID_Fatal, 'Error allocating DstParamData%forceBldEtaNodes.', ErrStat, ErrMsg,RoutineName)
      RETURN
    END IF
    DstParamData%c_obj%forceBldEtaNodes_Len = SIZE(DstParamData%forceBldEtaNodes)
    IF (DstParamData%c_obj%forceBldEtaNodes_Len > 0) &
          DstParamData%c_obj%forceBldEtaNodes = C_LOC( DstParamData%forceBldEtaNodes( i1_l ) )
  END IF
    DstParamData%forceBldEtaNodes = SrcParamData%forceBldEtaNodes
ENDIF
IF (ASSOCIATED(SrcParamData%forceTwrEtaNodes)) THEN
  i1_l = LBOUND(SrcParamData%forceTwrEtaNodes,1)
  i1_u = UBOUND(SrcParamData%forceTwrEtaNodes,1)
  IF (.NOT. ASSOCIATED(DstParamData%forceTwrEtaNodes)) THEN 
    ALLOCATE(DstParamData%forceTwrEtaNodes(i1_l:i1_u),STAT=ErrStat2)
    IF (ErrStat2 /= 0) THEN 
      CALL SetErrStat(ErrID_Fatal, 'Error allocating DstParamData%forceTwrEtaNodes.', ErrStat, ErrMsg,RoutineName)
      RETURN
    END IF
    DstParamData%c_obj%forceTwrEtaNodes_Len = SIZE(DstParamData%forceTwrEtaNodes)
    IF (DstParamData%c_obj%forceTwrEtaNodes_Len > 0) &
          DstParamData%c_obj%forceTwrEtaNodes = C_LOC( DstParamData%forceTwrEtaNodes( i1_l ) )
  END IF
    DstParamData%forceTwrEtaNodes = SrcParamData%forceTwrEtaNodes
ENDIF
 END SUBROUTINE ExtInfw_CopyParam

 SUBROUTINE ExtInfw_DestroyParam( ParamData, ErrStat, ErrMsg, DEALLOCATEpointers )
...
IF (ASSOCIATED(ParamData%forceBldEtaNodes)) THEN
 IF (DEALLOCATEpointers_local) &
  DEALLOCATE(ParamData%forceBldEtaNodes)
  ParamData%forceBldEtaNodes => NULL()
  ParamData%C_obj%forceBldEtaNodes = C_NULL_PTR
  ParamData%C_obj%forceBldEtaNodes_Len = 0
ENDIF
IF (ASSOCIATED(ParamData%forceTwrEtaNodes)) THEN
 IF (DEALLOCATEpointers_local) &
  DEALLOCATE(ParamData%forceTwrEtaNodes)
  ParamData%forceTwrEtaNodes => NULL()
  ParamData%C_obj%forceTwrEtaNodes = C_NULL_PTR
  ParamData%C_obj%forceTwrEtaNodes_Len = 0
ENDIF
 END SUBROUTINE ExtInfw_DestroyParam

 SUBROUTINE ExtInfw_PackParam( ReKiBuf, DbKiBuf, IntKiBuf, Indata, ErrStat, ErrMsg, SizeOnly )
  REAL(ReKi),       ALLOCATABLE, INTENT(  OUT) :: ReKiBuf(:)
  REAL(DbKi),       ALLOCATABLE, INTENT(  OUT) :: DbKiBuf(:)
  INTEGER(IntKi),   ALLOCATABLE, INTENT(  OUT) :: IntKiBuf(:)
 ...
 ! buffers to store subtypes, if any
  ...
  Int_BufSz   = Int_BufSz   + 1     ! forceBldEtaNodes allocated yes/no
  IF ( ASSOCIATED(InData%forceBldEtaNodes) ) THEN
    Int_BufSz   = Int_BufSz   + 2*1  ! forceBldEtaNodes upper/lower bounds for each dimension
      Re_BufSz   = Re_BufSz   + SIZE(InData%forceBldEtaNodes)  ! forceBldEtaNodes
  END IF
  Int_BufSz   = Int_BufSz   + 1     ! forceTwrEtaNodes allocated yes/no
  IF ( ASSOCIATED(InData%forceTwrEtaNodes) ) THEN
    Int_BufSz   = Int_BufSz   + 2*1  ! forceTwrEtaNodes upper/lower bounds for each dimension
      Re_BufSz   = Re_BufSz   + SIZE(InData%forceTwrEtaNodes)  ! forceTwrEtaNodes
  END IF
  IF ( Re_BufSz  .GT. 0 ) THEN 
     ALLOCATE( ReKiBuf(  Re_BufSz  ), STAT=ErrStat2 )
     IF (ErrStat2 /= 0) THEN 
  ...
  Int_Xferred = 1
    ...
    Int_Xferred = Int_Xferred + 1
  IF ( .NOT. ASSOCIATED(InData%forceBldEtaNodes) ) THEN
    IntKiBuf( Int_Xferred ) = 0
    Int_Xferred = Int_Xferred + 1
  ELSE
    IntKiBuf( Int_Xferred ) = 1
    Int_Xferred = Int_Xferred + 1
    IntKiBuf( Int_Xferred    ) = LBOUND(InData%forceBldEtaNodes,1)
    IntKiBuf( Int_Xferred + 1) = UBOUND(InData%forceBldEtaNodes,1)
    Int_Xferred = Int_Xferred + 2

      DO i1 = LBOUND(InData%forceBldEtaNodes,1), UBOUND(InData%forceBldEtaNodes,1)
        ReKiBuf(Re_Xferred) = InData%forceBldEtaNodes(i1)
        Re_Xferred = Re_Xferred + 1
      END DO
  END IF
  IF ( .NOT. ASSOCIATED(InData%forceTwrEtaNodes) ) THEN
    IntKiBuf( Int_Xferred ) = 0
    Int_Xferred = Int_Xferred + 1
  ELSE
    IntKiBuf( Int_Xferred ) = 1
    Int_Xferred = Int_Xferred + 1
    IntKiBuf( Int_Xferred    ) = LBOUND(InData%forceTwrEtaNodes,1)
    IntKiBuf( Int_Xferred + 1) = UBOUND(InData%forceTwrEtaNodes,1)
    Int_Xferred = Int_Xferred + 2

      DO i1 = LBOUND(InData%forceTwrEtaNodes,1), UBOUND(InData%forceTwrEtaNodes,1)
        ReKiBuf(Re_Xferred) = InData%forceTwrEtaNodes(i1)
        Re_Xferred = Re_Xferred + 1
      END DO
  END IF
 END SUBROUTINE ExtInfw_PackParam

 SUBROUTINE ExtInfw_UnPackParam( ReKiBuf, DbKiBuf, IntKiBuf, Outdata, ErrStat, ErrMsg )
  REAL(ReKi),      ALLOCATABLE, INTENT(IN   ) :: ReKiBuf(:)
  REAL(DbKi),      ALLOCATABLE, INTENT(IN   ) :: DbKiBuf(:)
  INTEGER(IntKi),  ALLOCATABLE, INTENT(IN   ) :: IntKiBuf(:)
 ...
 ! buffers to store meshes, if any
  ...
  IF ( IntKiBuf( Int_Xferred ) == 0 ) THEN  ! forceBldEtaNodes not allocated
    Int_Xferred = Int_Xferred + 1
  ELSE
    Int_Xferred = Int_Xferred + 1
    i1_l = IntKiBuf( Int_Xferred    )
    i1_u = IntKiBuf( Int_Xferred + 1)
    Int_Xferred = Int_Xferred + 2
    IF (ASSOCIATED(OutData%forceBldEtaNodes)) DEALLOCATE(OutData%forceBldEtaNodes)
    ALLOCATE(OutData%forceBldEtaNodes(i1_l:i1_u),STAT=ErrStat2)
    IF (ErrStat2 /= 0) THEN 
       CALL SetErrStat(ErrID_Fatal, 'Error allocating OutData%forceBldEtaNodes.', ErrStat, ErrMsg,RoutineName)
       RETURN
    END IF
    OutData%c_obj%forceBldEtaNodes_Len = SIZE(OutData%forceBldEtaNodes)
    IF (OutData%c_obj%forceBldEtaNodes_Len > 0) &
       OutData%c_obj%forceBldEtaNodes = C_LOC( OutData%forceBldEtaNodes( i1_l ) )
      DO i1 = LBOUND(OutData%forceBldEtaNodes,1), UBOUND(OutData%forceBldEtaNodes,1)
        OutData%forceBldEtaNodes(i1) = REAL(ReKiBuf(Re_Xferred), C_FLOAT)
        Re_Xferred = Re_Xferred + 1
      END DO
  END IF
  IF ( IntKiBuf( Int_Xferred ) == 0 ) THEN  ! forceTwrEtaNodes not allocated
    Int_Xferred = Int_Xferred + 1
  ELSE
    Int_Xferred = Int_Xferred + 1
    i1_l = IntKiBuf( Int_Xferred    )
    i1_u = IntKiBuf( Int_Xferred + 1)
    Int_Xferred = Int_Xferred + 2
    IF (ASSOCIATED(OutData%forceTwrEtaNodes)) DEALLOCATE(OutData%forceTwrEtaNodes)
    ALLOCATE(OutData%forceTwrEtaNodes(i1_l:i1_u),STAT=ErrStat2)
    IF (ErrStat2 /= 0) THEN 
       CALL SetErrStat(ErrID_Fatal, 'Error allocating OutData%forceTwrEtaNodes.', ErrStat, ErrMsg,RoutineName)
       RETURN
    END IF
    OutData%c_obj%forceTwrEtaNodes_Len = SIZE(OutData%forceTwrEtaNodes)
    IF (OutData%c_obj%forceTwrEtaNodes_Len > 0) &
       OutData%c_obj%forceTwrEtaNodes = C_LOC( OutData%forceTwrEtaNodes( i1_l ) )
      DO i1 = LBOUND(OutData%forceTwrEtaNodes,1), UBOUND(OutData%forceTwrEtaNodes,1)
        OutData%forceTwrEtaNodes(i1) = REAL(ReKiBuf(Re_Xferred), C_FLOAT)
        Re_Xferred = Re_Xferred + 1
      END DO
  END IF
 END SUBROUTINE ExtInfw_UnPackParam
 ...
 SUBROUTINE ExtInfw_C2Fary_CopyParam( ParamData, ErrStat, ErrMsg, SkipPointers )
    ...
    ! -- forceBldEtaNodes Param Data fields
    IF ( .NOT. SkipPointers_local ) THEN
       IF ( .NOT. C_ASSOCIATED( ParamData%C_obj%forceBldEtaNodes ) ) THEN
          NULLIFY( ParamData%forceBldEtaNodes )
       ELSE
          CALL C_F_POINTER(ParamData%C_obj%forceBldEtaNodes, ParamData%forceBldEtaNodes, (/ParamData%C_obj%forceBldEtaNodes_Len/))
       END IF
    END IF

    ! -- forceTwrEtaNodes Param Data fields
    IF ( .NOT. SkipPointers_local ) THEN
       IF ( .NOT. C_ASSOCIATED( ParamData%C_obj%forceTwrEtaNodes ) ) THEN
          NULLIFY( ParamData%forceTwrEtaNodes )
       ELSE
          CALL C_F_POINTER(ParamData%C_obj%forceTwrEtaNodes, ParamData%forceTwrEtaNodes, (/ParamData%C_obj%forceTwrEtaNodes_Len/))
       END IF
    END IF
 END SUBROUTINE ExtInfw_C2Fary_CopyParam
 ...
 SUBROUTINE ExtInfw_F2C_CopyParam( ParamData, ErrStat, ErrMsg, SkipPointers  )
    ...
    ! -- forceBldEtaNodes Param Data fields
    IF ( .NOT. SkipPointers_local ) THEN
       IF ( .NOT. ASSOCIATED(ParamData%forceBldEtaNodes)) THEN 
          ParamData%c_obj%forceBldEtaNodes_Len = 0
          ParamData%c_obj%forceBldEtaNodes = C_NULL_PTR
       ELSE
          ParamData%c_obj%forceBldEtaNodes_Len = SIZE(ParamData%forceBldEtaNodes)
          IF (ParamData%c_obj%forceBldEtaNodes_Len > 0) &
             ParamData%c_obj%forceBldEtaNodes = C_LOC( ParamData%forceBldEtaNodes( LBOUND(ParamData%forceBldEtaNodes,1) ) )
       END IF
    END IF

    ! -- forceTwrEtaNodes Param Data fields
    IF ( .NOT. SkipPointers_local ) THEN
       IF ( .NOT. ASSOCIATED(ParamData%forceTwrEtaNodes)) THEN 
          ParamData%c_obj%forceTwrEtaNodes_Len = 0
          ParamData%c_obj%forceTwrEtaNodes = C_NULL_PTR
       ELSE
          ParamData%c_obj%forceTwrEtaNodes_Len = SIZE(ParamData%forceTwrEtaNodes)
          IF (ParamData%c_obj%forceTwrEtaNodes_Len > 0) &
             ParamData%c_obj%forceTwrEtaNodes = C_LOC( ParamData%forceTwrEtaNodes( LBOUND(ParamData%forceTwrEtaNodes,1) ) )
       END IF
    END IF
 END SUBROUTINE ExtInfw_F2C_CopyParam
...
ENDIF
IF (ASSOCIATED(SrcInputData%pxdotVel)) THEN
  i1_l = LBOUND(SrcInputData%pxdotVel,1)
  i1_u = UBOUND(SrcInputData%pxdotVel,1)
  IF (.NOT. ASSOCIATED(DstInputData%pxdotVel)) THEN 
    ALLOCATE(DstInputData%pxdotVel(i1_l:i1_u),STAT=ErrStat2)
    IF (ErrStat2 /= 0) THEN 
      CALL SetErrStat(ErrID_Fatal, 'Error allocating DstInputData%pxdotVel.', ErrStat, ErrMsg,RoutineName)
      RETURN
    END IF
    DstInputData%c_obj%pxdotVel_Len = SIZE(DstInputData%pxdotVel)
    IF (DstInputData%c_obj%pxdotVel_Len > 0) &
          DstInputData%c_obj%pxdotVel = C_LOC( DstInputData%pxdotVel( i1_l ) )
  END IF
    DstInputData%pxdotVel = SrcInputData%pxdotVel
ENDIF
IF (ASSOCIATED(SrcInputData%pydotVel)) THEN
  i1_l = LBOUND(SrcInputData%pydotVel,1)
  i1_u = UBOUND(SrcInputData%pydotVel,1)
  IF (.NOT. ASSOCIATED(DstInputData%pydotVel)) THEN 
    ALLOCATE(DstInputData%pydotVel(i1_l:i1_u),STAT=ErrStat2)
    IF (ErrStat2 /= 0) THEN 
      CALL SetErrStat(ErrID_Fatal, 'Error allocating DstInputData%pydotVel.', ErrStat, ErrMsg,RoutineName)
      RETURN
    END IF
    DstInputData%c_obj%pydotVel_Len = SIZE(DstInputData%pydotVel)
    IF (DstInputData%c_obj%pydotVel_Len > 0) &
          DstInputData%c_obj%pydotVel = C_LOC( DstInputData%pydotVel( i1_l ) )
  END IF
    DstInputData%pydotVel = SrcInputData%pydotVel
ENDIF
IF (ASSOCIATED(SrcInputData%pzdotVel)) THEN
  i1_l = LBOUND(SrcInputData%pzdotVel,1)
  i1_u = UBOUND(SrcInputData%pzdotVel,1)
  IF (.NOT. ASSOCIATED(DstInputData%pzdotVel)) THEN 
    ALLOCATE(DstInputData%pzdotVel(i1_l:i1_u),STAT=ErrStat2)
    IF (ErrStat2 /= 0) THEN 
      CALL SetErrStat(ErrID_Fatal, 'Error allocating DstInputData%pzdotVel.', ErrStat, ErrMsg,RoutineName)
      RETURN
    END IF
    DstInputData%c_obj%pzdotVel_Len = SIZE(DstInputData%pzdotVel)
    IF (DstInputData%c_obj%pzdotVel_Len > 0) &
          DstInputData%c_obj%pzdotVel = C_LOC( DstInputData%pzdotVel( i1_l ) )
  END IF
    DstInputData%pzdotVel = SrcInputData%pzdotVel
ENDIF
...
ENDIF
IF (ASSOCIATED(SrcInputData%pxdotForce)) THEN
  i1_l = LBOUND(SrcInputData%pxdotForce,1)
  i1_u = UBOUND(SrcInputData%pxdotForce,1)
  IF (.NOT. ASSOCIATED(DstInputData%pxdotForce)) THEN 
    ALLOCATE(DstInputData%pxdotForce(i1_l:i1_u),STAT=ErrStat2)
    IF (ErrStat2 /= 0) THEN 
      CALL SetErrStat(ErrID_Fatal, 'Error allocating DstInputData%pxdotForce.', ErrStat, ErrMsg,RoutineName)
      RETURN
    END IF
    DstInputData%c_obj%pxdotForce_Len = SIZE(DstInputData%pxdotForce)
    IF (DstInputData%c_obj%pxdotForce_Len > 0) &
          DstInputData%c_obj%pxdotForce = C_LOC( DstInputData%pxdotForce( i1_l ) )
  END IF
    DstInputData%pxdotForce = SrcInputData%pxdotForce
ENDIF
IF (ASSOCIATED(SrcInputData%pydotForce)) THEN
  i1_l = LBOUND(SrcInputData%pydotForce,1)
  i1_u = UBOUND(SrcInputData%pydotForce,1)
  IF (.NOT. ASSOCIATED(DstInputData%pydotForce)) THEN 
    ALLOCATE(DstInputData%pydotForce(i1_l:i1_u),STAT=ErrStat2)
    IF (ErrStat2 /= 0) THEN 
      CALL SetErrStat(ErrID_Fatal, 'Error allocating DstInputData%pydotForce.', ErrStat, ErrMsg,RoutineName)
      RETURN
    END IF
    DstInputData%c_obj%pydotForce_Len = SIZE(DstInputData%pydotForce)
    IF (DstInputData%c_obj%pydotForce_Len > 0) &
          DstInputData%c_obj%pydotForce = C_LOC( DstInputData%pydotForce( i1_l ) )
  END IF
    DstInputData%pydotForce = SrcInputData%pydotForce
ENDIF
IF (ASSOCIATED(SrcInputData%pzdotForce)) THEN
  i1_l = LBOUND(SrcInputData%pzdotForce,1)
  i1_u = UBOUND(SrcInputData%pzdotForce,1)
  IF (.NOT. ASSOCIATED(DstInputData%pzdotForce)) THEN 
    ALLOCATE(DstInputData%pzdotForce(i1_l:i1_u),STAT=ErrStat2)
    IF (ErrStat2 /= 0) THEN 
      CALL SetErrStat(ErrID_Fatal, 'Error allocating DstInputData%pzdotForce.', ErrStat, ErrMsg,RoutineName)
      RETURN
    END IF
    DstInputData%c_obj%pzdotForce_Len = SIZE(DstInputData%pzdotForce)
    IF (DstInputData%c_obj%pzdotForce_Len > 0) &
          DstInputData%c_obj%pzdotForce = C_LOC( DstInputData%pzdotForce( i1_l ) )
  END IF
    DstInputData%pzdotForce = SrcInputData%pzdotForce
ENDIF
...
ENDIF
 END SUBROUTINE ExtInfw_CopyInput

 SUBROUTINE ExtInfw_DestroyInput( InputData, ErrStat, ErrMsg, DEALLOCATEpointers )
...
ENDIF
IF (ASSOCIATED(InputData%pxdotVel)) THEN
 IF (DEALLOCATEpointers_local) &
  DEALLOCATE(InputData%pxdotVel)
  InputData%pxdotVel => NULL()
  InputData%C_obj%pxdotVel = C_NULL_PTR
  InputData%C_obj%pxdotVel_Len = 0
ENDIF
IF (ASSOCIATED(InputData%pydotVel)) THEN
 IF (DEALLOCATEpointers_local) &
  DEALLOCATE(InputData%pydotVel)
  InputData%pydotVel => NULL()
  InputData%C_obj%pydotVel = C_NULL_PTR
  InputData%C_obj%pydotVel_Len = 0
ENDIF
IF (ASSOCIATED(InputData%pzdotVel)) THEN
 IF (DEALLOCATEpointers_local) &
  DEALLOCATE(InputData%pzdotVel)
  InputData%pzdotVel => NULL()
  InputData%C_obj%pzdotVel = C_NULL_PTR
  InputData%C_obj%pzdotVel_Len = 0
ENDIF
...
ENDIF
IF (ASSOCIATED(InputData%pxdotForce)) THEN
 IF (DEALLOCATEpointers_local) &
  DEALLOCATE(InputData%pxdotForce)
  InputData%pxdotForce => NULL()
  InputData%C_obj%pxdotForce = C_NULL_PTR
  InputData%C_obj%pxdotForce_Len = 0
ENDIF
IF (ASSOCIATED(InputData%pydotForce)) THEN
 IF (DEALLOCATEpointers_local) &
  DEALLOCATE(InputData%pydotForce)
  InputData%pydotForce => NULL()
  InputData%C_obj%pydotForce = C_NULL_PTR
  InputData%C_obj%pydotForce_Len = 0
ENDIF
IF (ASSOCIATED(InputData%pzdotForce)) THEN
 IF (DEALLOCATEpointers_local) &
  DEALLOCATE(InputData%pzdotForce)
  InputData%pzdotForce => NULL()
  InputData%C_obj%pzdotForce = C_NULL_PTR
  InputData%C_obj%pzdotForce_Len = 0
ENDIF
...
ENDIF
 END SUBROUTINE ExtInfw_DestroyInput

 SUBROUTINE ExtInfw_PackInput( ReKiBuf, DbKiBuf, IntKiBuf, Indata, ErrStat, ErrMsg, SizeOnly )
  REAL(ReKi),       ALLOCATABLE, INTENT(  OUT) :: ReKiBuf(:)
  REAL(DbKi),       ALLOCATABLE, INTENT(  OUT) :: DbKiBuf(:)
  INTEGER(IntKi),   ALLOCATABLE, INTENT(  OUT) :: IntKiBuf(:)
 ...
 ! buffers to store subtypes, if any
  ...
  END IF
  Int_BufSz   = Int_BufSz   + 1     ! pxdotVel allocated yes/no
  IF ( ASSOCIATED(InData%pxdotVel) ) THEN
    Int_BufSz   = Int_BufSz   + 2*1  ! pxdotVel upper/lower bounds for each dimension
      Re_BufSz   = Re_BufSz   + SIZE(InData%pxdotVel)  ! pxdotVel
  END IF
  Int_BufSz   = Int_BufSz   + 1     ! pydotVel allocated yes/no
  IF ( ASSOCIATED(InData%pydotVel) ) THEN
    Int_BufSz   = Int_BufSz   + 2*1  ! pydotVel upper/lower bounds for each dimension
      Re_BufSz   = Re_BufSz   + SIZE(InData%pydotVel)  ! pydotVel
  END IF
  Int_BufSz   = Int_BufSz   + 1     ! pzdotVel allocated yes/no
  IF ( ASSOCIATED(InData%pzdotVel) ) THEN
    Int_BufSz   = Int_BufSz   + 2*1  ! pzdotVel upper/lower bounds for each dimension
      Re_BufSz   = Re_BufSz   + SIZE(InData%pzdotVel)  ! pzdotVel
  END IF
  ...
  END IF
  Int_BufSz   = Int_BufSz   + 1     ! pxdotForce allocated yes/no
  IF ( ASSOCIATED(InData%pxdotForce) ) THEN
    Int_BufSz   = Int_BufSz   + 2*1  ! pxdotForce upper/lower bounds for each dimension
      Re_BufSz   = Re_BufSz   + SIZE(InData%pxdotForce)  ! pxdotForce
  END IF
  Int_BufSz   = Int_BufSz   + 1     ! pydotForce allocated yes/no
  IF ( ASSOCIATED(InData%pydotForce) ) THEN
    Int_BufSz   = Int_BufSz   + 2*1  ! pydotForce upper/lower bounds for each dimension
      Re_BufSz   = Re_BufSz   + SIZE(InData%pydotForce)  ! pydotForce
  END IF
  Int_BufSz   = Int_BufSz   + 1     ! pzdotForce allocated yes/no
  IF ( ASSOCIATED(InData%pzdotForce) ) THEN
    Int_BufSz   = Int_BufSz   + 2*1  ! pzdotForce upper/lower bounds for each dimension
      Re_BufSz   = Re_BufSz   + SIZE(InData%pzdotForce)  ! pzdotForce
  END IF
  ...
  ELSE
    ...
    Int_Xferred = Int_Xferred + 2
      ...
      END DO
  END IF
  IF ( .NOT. ASSOCIATED(InData%pxdotVel) ) THEN
    IntKiBuf( Int_Xferred ) = 0
    Int_Xferred = Int_Xferred + 1
  ELSE
    IntKiBuf( Int_Xferred ) = 1
    Int_Xferred = Int_Xferred + 1
    IntKiBuf( Int_Xferred    ) = LBOUND(InData%pxdotVel,1)
    IntKiBuf( Int_Xferred + 1) = UBOUND(InData%pxdotVel,1)
    Int_Xferred = Int_Xferred + 2

      DO i1 = LBOUND(InData%pxdotVel,1), UBOUND(InData%pxdotVel,1)
        ReKiBuf(Re_Xferred) = InData%pxdotVel(i1)
        Re_Xferred = Re_Xferred + 1
      END DO
  END IF
  IF ( .NOT. ASSOCIATED(InData%pydotVel) ) THEN
    IntKiBuf( Int_Xferred ) = 0
    Int_Xferred = Int_Xferred + 1
  ELSE
    IntKiBuf( Int_Xferred ) = 1
    Int_Xferred = Int_Xferred + 1
    IntKiBuf( Int_Xferred    ) = LBOUND(InData%pydotVel,1)
    IntKiBuf( Int_Xferred + 1) = UBOUND(InData%pydotVel,1)
    Int_Xferred = Int_Xferred + 2

      DO i1 = LBOUND(InData%pydotVel,1), UBOUND(InData%pydotVel,1)
        ReKiBuf(Re_Xferred) = InData%pydotVel(i1)
        Re_Xferred = Re_Xferred + 1
      END DO
  END IF
  IF ( .NOT. ASSOCIATED(InData%pzdotVel) ) THEN
    IntKiBuf( Int_Xferred ) = 0
    Int_Xferred = Int_Xferred + 1
  ELSE
    IntKiBuf( Int_Xferred ) = 1
    Int_Xferred = Int_Xferred + 1
    IntKiBuf( Int_Xferred    ) = LBOUND(InData%pzdotVel,1)
    IntKiBuf( Int_Xferred + 1) = UBOUND(InData%pzdotVel,1)
    Int_Xferred = Int_Xferred + 2

      DO i1 = LBOUND(InData%pzdotVel,1), UBOUND(InData%pzdotVel,1)
        ReKiBuf(Re_Xferred) = InData%pzdotVel(i1)
        Re_Xferred = Re_Xferred + 1
      END DO
  END IF
  ...
  ELSE
    ...
    Int_Xferred = Int_Xferred + 2
      ...
      END DO
  END IF
  IF ( .NOT. ASSOCIATED(InData%pxdotForce) ) THEN
    IntKiBuf( Int_Xferred ) = 0
    Int_Xferred = Int_Xferred + 1
  ELSE
    IntKiBuf( Int_Xferred ) = 1
    Int_Xferred = Int_Xferred + 1
    IntKiBuf( Int_Xferred    ) = LBOUND(InData%pxdotForce,1)
    IntKiBuf( Int_Xferred + 1) = UBOUND(InData%pxdotForce,1)
    Int_Xferred = Int_Xferred + 2

      DO i1 = LBOUND(InData%pxdotForce,1), UBOUND(InData%pxdotForce,1)
        ReKiBuf(Re_Xferred) = InData%pxdotForce(i1)
        Re_Xferred = Re_Xferred + 1
      END DO
  END IF
  IF ( .NOT. ASSOCIATED(InData%pydotForce) ) THEN
    IntKiBuf( Int_Xferred ) = 0
    Int_Xferred = Int_Xferred + 1
  ELSE
    IntKiBuf( Int_Xferred ) = 1
    Int_Xferred = Int_Xferred + 1
    IntKiBuf( Int_Xferred    ) = LBOUND(InData%pydotForce,1)
    IntKiBuf( Int_Xferred + 1) = UBOUND(InData%pydotForce,1)
    Int_Xferred = Int_Xferred + 2

      DO i1 = LBOUND(InData%pydotForce,1), UBOUND(InData%pydotForce,1)
        ReKiBuf(Re_Xferred) = InData%pydotForce(i1)
        Re_Xferred = Re_Xferred + 1
      END DO
  END IF
  IF ( .NOT. ASSOCIATED(InData%pzdotForce) ) THEN
    IntKiBuf( Int_Xferred ) = 0
    Int_Xferred = Int_Xferred + 1
  ELSE
    IntKiBuf( Int_Xferred ) = 1
    Int_Xferred = Int_Xferred + 1
    IntKiBuf( Int_Xferred    ) = LBOUND(InData%pzdotForce,1)
    IntKiBuf( Int_Xferred + 1) = UBOUND(InData%pzdotForce,1)
    Int_Xferred = Int_Xferred + 2

      DO i1 = LBOUND(InData%pzdotForce,1), UBOUND(InData%pzdotForce,1)
        ReKiBuf(Re_Xferred) = InData%pzdotForce(i1)
        Re_Xferred = Re_Xferred + 1
      END DO
  END IF
  ...
  ELSE
    ...
    Int_Xferred = Int_Xferred + 2
      ...
      END DO
  END IF
 END SUBROUTINE ExtInfw_PackInput

 SUBROUTINE ExtInfw_UnPackInput( ReKiBuf, DbKiBuf, IntKiBuf, Outdata, ErrStat, ErrMsg )
  REAL(ReKi),      ALLOCATABLE, INTENT(IN   ) :: ReKiBuf(:)
  REAL(DbKi),      ALLOCATABLE, INTENT(IN   ) :: DbKiBuf(:)
  INTEGER(IntKi),  ALLOCATABLE, INTENT(IN   ) :: IntKiBuf(:)
 ...
 ! buffers to store meshes, if any
  ...
  ELSE
    ...
    IF (OutData%c_obj%pzVel_Len > 0) &
      ...
      END DO
  END IF
  IF ( IntKiBuf( Int_Xferred ) == 0 ) THEN  ! pxdotVel not allocated
    Int_Xferred = Int_Xferred + 1
  ELSE
    Int_Xferred = Int_Xferred + 1
    i1_l = IntKiBuf( Int_Xferred    )
    i1_u = IntKiBuf( Int_Xferred + 1)
    Int_Xferred = Int_Xferred + 2
    IF (ASSOCIATED(OutData%pxdotVel)) DEALLOCATE(OutData%pxdotVel)
    ALLOCATE(OutData%pxdotVel(i1_l:i1_u),STAT=ErrStat2)
    IF (ErrStat2 /= 0) THEN 
       CALL SetErrStat(ErrID_Fatal, 'Error allocating OutData%pxdotVel.', ErrStat, ErrMsg,RoutineName)
       RETURN
    END IF
    OutData%c_obj%pxdotVel_Len = SIZE(OutData%pxdotVel)
    IF (OutData%c_obj%pxdotVel_Len > 0) &
       OutData%c_obj%pxdotVel = C_LOC( OutData%pxdotVel( i1_l ) )
      DO i1 = LBOUND(OutData%pxdotVel,1), UBOUND(OutData%pxdotVel,1)
        OutData%pxdotVel(i1) = REAL(ReKiBuf(Re_Xferred), C_FLOAT)
        Re_Xferred = Re_Xferred + 1
      END DO
  END IF
  IF ( IntKiBuf( Int_Xferred ) == 0 ) THEN  ! pydotVel not allocated
    Int_Xferred = Int_Xferred + 1
  ELSE
    Int_Xferred = Int_Xferred + 1
    i1_l = IntKiBuf( Int_Xferred    )
    i1_u = IntKiBuf( Int_Xferred + 1)
    Int_Xferred = Int_Xferred + 2
    IF (ASSOCIATED(OutData%pydotVel)) DEALLOCATE(OutData%pydotVel)
    ALLOCATE(OutData%pydotVel(i1_l:i1_u),STAT=ErrStat2)
    IF (ErrStat2 /= 0) THEN 
       CALL SetErrStat(ErrID_Fatal, 'Error allocating OutData%pydotVel.', ErrStat, ErrMsg,RoutineName)
       RETURN
    END IF
    OutData%c_obj%pydotVel_Len = SIZE(OutData%pydotVel)
    IF (OutData%c_obj%pydotVel_Len > 0) &
       OutData%c_obj%pydotVel = C_LOC( OutData%pydotVel( i1_l ) )
      DO i1 = LBOUND(OutData%pydotVel,1), UBOUND(OutData%pydotVel,1)
        OutData%pydotVel(i1) = REAL(ReKiBuf(Re_Xferred), C_FLOAT)
        Re_Xferred = Re_Xferred + 1
      END DO
  END IF
  IF ( IntKiBuf( Int_Xferred ) == 0 ) THEN  ! pzdotVel not allocated
    Int_Xferred = Int_Xferred + 1
  ELSE
    Int_Xferred = Int_Xferred + 1
    i1_l = IntKiBuf( Int_Xferred    )
    i1_u = IntKiBuf( Int_Xferred + 1)
    Int_Xferred = Int_Xferred + 2
    IF (ASSOCIATED(OutData%pzdotVel)) DEALLOCATE(OutData%pzdotVel)
    ALLOCATE(OutData%pzdotVel(i1_l:i1_u),STAT=ErrStat2)
    IF (ErrStat2 /= 0) THEN 
       CALL SetErrStat(ErrID_Fatal, 'Error allocating OutData%pzdotVel.', ErrStat, ErrMsg,RoutineName)
       RETURN
    END IF
    OutData%c_obj%pzdotVel_Len = SIZE(OutData%pzdotVel)
    IF (OutData%c_obj%pzdotVel_Len > 0) &
       OutData%c_obj%pzdotVel = C_LOC( OutData%pzdotVel( i1_l ) )
      DO i1 = LBOUND(OutData%pzdotVel,1), UBOUND(OutData%pzdotVel,1)
        OutData%pzdotVel(i1) = REAL(ReKiBuf(Re_Xferred), C_FLOAT)
        Re_Xferred = Re_Xferred + 1
      END DO
  END IF
  ...
  ELSE
    ...
    IF (OutData%c_obj%pzForce_Len > 0) &
      ...
      END DO
  END IF
  IF ( IntKiBuf( Int_Xferred ) == 0 ) THEN  ! pxdotForce not allocated
    Int_Xferred = Int_Xferred + 1
  ELSE
    Int_Xferred = Int_Xferred + 1
    i1_l = IntKiBuf( Int_Xferred    )
    i1_u = IntKiBuf( Int_Xferred + 1)
    Int_Xferred = Int_Xferred + 2
    IF (ASSOCIATED(OutData%pxdotForce)) DEALLOCATE(OutData%pxdotForce)
    ALLOCATE(OutData%pxdotForce(i1_l:i1_u),STAT=ErrStat2)
    IF (ErrStat2 /= 0) THEN 
       CALL SetErrStat(ErrID_Fatal, 'Error allocating OutData%pxdotForce.', ErrStat, ErrMsg,RoutineName)
       RETURN
    END IF
    OutData%c_obj%pxdotForce_Len = SIZE(OutData%pxdotForce)
    IF (OutData%c_obj%pxdotForce_Len > 0) &
       OutData%c_obj%pxdotForce = C_LOC( OutData%pxdotForce( i1_l ) )
      DO i1 = LBOUND(OutData%pxdotForce,1), UBOUND(OutData%pxdotForce,1)
        OutData%pxdotForce(i1) = REAL(ReKiBuf(Re_Xferred), C_FLOAT)
        Re_Xferred = Re_Xferred + 1
      END DO
  END IF
  IF ( IntKiBuf( Int_Xferred ) == 0 ) THEN  ! pydotForce not allocated
    Int_Xferred = Int_Xferred + 1
  ELSE
    Int_Xferred = Int_Xferred + 1
    i1_l = IntKiBuf( Int_Xferred    )
    i1_u = IntKiBuf( Int_Xferred + 1)
    Int_Xferred = Int_Xferred + 2
    IF (ASSOCIATED(OutData%pydotForce)) DEALLOCATE(OutData%pydotForce)
    ALLOCATE(OutData%pydotForce(i1_l:i1_u),STAT=ErrStat2)
    IF (ErrStat2 /= 0) THEN 
       CALL SetErrStat(ErrID_Fatal, 'Error allocating OutData%pydotForce.', ErrStat, ErrMsg,RoutineName)
       RETURN
    END IF
    OutData%c_obj%pydotForce_Len = SIZE(OutData%pydotForce)
    IF (OutData%c_obj%pydotForce_Len > 0) &
       OutData%c_obj%pydotForce = C_LOC( OutData%pydotForce( i1_l ) )
      DO i1 = LBOUND(OutData%pydotForce,1), UBOUND(OutData%pydotForce,1)
        OutData%pydotForce(i1) = REAL(ReKiBuf(Re_Xferred), C_FLOAT)
        Re_Xferred = Re_Xferred + 1
      END DO
  END IF
  IF ( IntKiBuf( Int_Xferred ) == 0 ) THEN  ! pzdotForce not allocated
    Int_Xferred = Int_Xferred + 1
  ELSE
    Int_Xferred = Int_Xferred + 1
    i1_l = IntKiBuf( Int_Xferred    )
    i1_u = IntKiBuf( Int_Xferred + 1)
    Int_Xferred = Int_Xferred + 2
    IF (ASSOCIATED(OutData%pzdotForce)) DEALLOCATE(OutData%pzdotForce)
    ALLOCATE(OutData%pzdotForce(i1_l:i1_u),STAT=ErrStat2)
    IF (ErrStat2 /= 0) THEN 
       CALL SetErrStat(ErrID_Fatal, 'Error allocating OutData%pzdotForce.', ErrStat, ErrMsg,RoutineName)
       RETURN
    END IF
    OutData%c_obj%pzdotForce_Len = SIZE(OutData%pzdotForce)
    IF (OutData%c_obj%pzdotForce_Len > 0) &
       OutData%c_obj%pzdotForce = C_LOC( OutData%pzdotForce( i1_l ) )
      DO i1 = LBOUND(OutData%pzdotForce,1), UBOUND(OutData%pzdotForce,1)
        OutData%pzdotForce(i1) = REAL(ReKiBuf(Re_Xferred), C_FLOAT)
        Re_Xferred = Re_Xferred + 1
      END DO
  END IF
  ...
  ELSE
    ...
    IF (OutData%c_obj%forceNodesChord_Len > 0) &
      ...
      END DO
  END IF
 END SUBROUTINE ExtInfw_UnPackInput
 ...
 SUBROUTINE ExtInfw_C2Fary_CopyInput( InputData, ErrStat, ErrMsg, SkipPointers )
    ...
    ! -- pxdotVel Input Data fields
    IF ( .NOT. SkipPointers_local ) THEN
       IF ( .NOT. C_ASSOCIATED( InputData%C_obj%pxdotVel ) ) THEN
          NULLIFY( InputData%pxdotVel )
       ELSE
          CALL C_F_POINTER(InputData%C_obj%pxdotVel, InputData%pxdotVel, (/InputData%C_obj%pxdotVel_Len/))
       END IF
    END IF

    ! -- pydotVel Input Data fields
    IF ( .NOT. SkipPointers_local ) THEN
       IF ( .NOT. C_ASSOCIATED( InputData%C_obj%pydotVel ) ) THEN
          NULLIFY( InputData%pydotVel )
       ELSE
          CALL C_F_POINTER(InputData%C_obj%pydotVel, InputData%pydotVel, (/InputData%C_obj%pydotVel_Len/))
       END IF
    END IF

    ! -- pzdotVel Input Data fields
    IF ( .NOT. SkipPointers_local ) THEN
       IF ( .NOT. C_ASSOCIATED( InputData%C_obj%pzdotVel ) ) THEN
          NULLIFY( InputData%pzdotVel )
       ELSE
          CALL C_F_POINTER(InputData%C_obj%pzdotVel, InputData%pzdotVel, (/InputData%C_obj%pzdotVel_Len/))
       END IF
    END IF
    ...
    ! -- pxdotForce Input Data fields
    IF ( .NOT. SkipPointers_local ) THEN
       IF ( .NOT. C_ASSOCIATED( InputData%C_obj%pxdotForce ) ) THEN
          NULLIFY( InputData%pxdotForce )
       ELSE
          CALL C_F_POINTER(InputData%C_obj%pxdotForce, InputData%pxdotForce, (/InputData%C_obj%pxdotForce_Len/))
       END IF
    END IF

    ! -- pydotForce Input Data fields
    IF ( .NOT. SkipPointers_local ) THEN
       IF ( .NOT. C_ASSOCIATED( InputData%C_obj%pydotForce ) ) THEN
          NULLIFY( InputData%pydotForce )
       ELSE
          CALL C_F_POINTER(InputData%C_obj%pydotForce, InputData%pydotForce, (/InputData%C_obj%pydotForce_Len/))
       END IF
    END IF

    ! -- pzdotForce Input Data fields
    IF ( .NOT. SkipPointers_local ) THEN
       IF ( .NOT. C_ASSOCIATED( InputData%C_obj%pzdotForce ) ) THEN
          NULLIFY( InputData%pzdotForce )
       ELSE
          CALL C_F_POINTER(InputData%C_obj%pzdotForce, InputData%pzdotForce, (/InputData%C_obj%pzdotForce_Len/))
       END IF
    END IF
    ...
    IF ( .NOT. SkipPointers_local ) THEN
       ...
       END IF
    END IF
 END SUBROUTINE ExtInfw_C2Fary_CopyInput
 ...
 SUBROUTINE ExtInfw_F2C_CopyInput( InputData, ErrStat, ErrMsg, SkipPointers  )
    ...
    ! -- pxdotVel Input Data fields
    IF ( .NOT. SkipPointers_local ) THEN
       IF ( .NOT. ASSOCIATED(InputData%pxdotVel)) THEN 
          InputData%c_obj%pxdotVel_Len = 0
          InputData%c_obj%pxdotVel = C_NULL_PTR
       ELSE
          InputData%c_obj%pxdotVel_Len = SIZE(InputData%pxdotVel)
          IF (InputData%c_obj%pxdotVel_Len > 0) &
             InputData%c_obj%pxdotVel = C_LOC( InputData%pxdotVel( LBOUND(InputData%pxdotVel,1) ) )
       END IF
    END IF

    ! -- pydotVel Input Data fields
    IF ( .NOT. SkipPointers_local ) THEN
       IF ( .NOT. ASSOCIATED(InputData%pydotVel)) THEN 
          InputData%c_obj%pydotVel_Len = 0
          InputData%c_obj%pydotVel = C_NULL_PTR
       ELSE
          InputData%c_obj%pydotVel_Len = SIZE(InputData%pydotVel)
          IF (InputData%c_obj%pydotVel_Len > 0) &
             InputData%c_obj%pydotVel = C_LOC( InputData%pydotVel( LBOUND(InputData%pydotVel,1) ) )
       END IF
    END IF

    ! -- pzdotVel Input Data fields
    IF ( .NOT. SkipPointers_local ) THEN
       IF ( .NOT. ASSOCIATED(InputData%pzdotVel)) THEN 
          InputData%c_obj%pzdotVel_Len = 0
          InputData%c_obj%pzdotVel = C_NULL_PTR
       ELSE
          InputData%c_obj%pzdotVel_Len = SIZE(InputData%pzdotVel)
          IF (InputData%c_obj%pzdotVel_Len > 0) &
             InputData%c_obj%pzdotVel = C_LOC( InputData%pzdotVel( LBOUND(InputData%pzdotVel,1) ) )
       END IF
    END IF
    ...
    ! -- pxdotForce Input Data fields
    IF ( .NOT. SkipPointers_local ) THEN
       IF ( .NOT. ASSOCIATED(InputData%pxdotForce)) THEN 
          InputData%c_obj%pxdotForce_Len = 0
          InputData%c_obj%pxdotForce = C_NULL_PTR
       ELSE
          InputData%c_obj%pxdotForce_Len = SIZE(InputData%pxdotForce)
          IF (InputData%c_obj%pxdotForce_Len > 0) &
             InputData%c_obj%pxdotForce = C_LOC( InputData%pxdotForce( LBOUND(InputData%pxdotForce,1) ) )
       END IF
    END IF

    ! -- pydotForce Input Data fields
    IF ( .NOT. SkipPointers_local ) THEN
       IF ( .NOT. ASSOCIATED(InputData%pydotForce)) THEN 
          InputData%c_obj%pydotForce_Len = 0
          InputData%c_obj%pydotForce = C_NULL_PTR
       ELSE
          InputData%c_obj%pydotForce_Len = SIZE(InputData%pydotForce)
          IF (InputData%c_obj%pydotForce_Len > 0) &
             InputData%c_obj%pydotForce = C_LOC( InputData%pydotForce( LBOUND(InputData%pydotForce,1) ) )
       END IF
    END IF

    ! -- pzdotForce Input Data fields
    IF ( .NOT. SkipPointers_local ) THEN
       IF ( .NOT. ASSOCIATED(InputData%pzdotForce)) THEN 
          InputData%c_obj%pzdotForce_Len = 0
          InputData%c_obj%pzdotForce = C_NULL_PTR
       ELSE
          InputData%c_obj%pzdotForce_Len = SIZE(InputData%pzdotForce)
          IF (InputData%c_obj%pzdotForce_Len > 0) &
             InputData%c_obj%pzdotForce = C_LOC( InputData%pzdotForce( LBOUND(InputData%pzdotForce,1) ) )
       END IF
    END IF
    ...
    IF ( .NOT. SkipPointers_local ) THEN
       ...
       END IF
    END IF
 END SUBROUTINE ExtInfw_F2C_CopyInput
...
ENDIF
 END SUBROUTINE ExtInfw_CopyOutput

 SUBROUTINE ExtInfw_DestroyOutput( OutputData, ErrStat, ErrMsg, DEALLOCATEpointers )
...
ENDIF
 END SUBROUTINE ExtInfw_DestroyOutput

 SUBROUTINE ExtInfw_PackOutput( ReKiBuf, DbKiBuf, IntKiBuf, Indata, ErrStat, ErrMsg, SizeOnly )
  REAL(ReKi),       ALLOCATABLE, INTENT(  OUT) :: ReKiBuf(:)
  REAL(DbKi),       ALLOCATABLE, INTENT(  OUT) :: DbKiBuf(:)
  INTEGER(IntKi),   ALLOCATABLE, INTENT(  OUT) :: IntKiBuf(:)
 ...
 ! buffers to store subtypes, if any
  ...
  ELSE
    ...
    Int_Xferred = Int_Xferred + 2
      ...
      END DO
  END IF
 END SUBROUTINE ExtInfw_PackOutput

 SUBROUTINE ExtInfw_UnPackOutput( ReKiBuf, DbKiBuf, IntKiBuf, Outdata, ErrStat, ErrMsg )
  REAL(ReKi),      ALLOCATABLE, INTENT(IN   ) :: ReKiBuf(:)
  REAL(DbKi),      ALLOCATABLE, INTENT(IN   ) :: DbKiBuf(:)
  INTEGER(IntKi),  ALLOCATABLE, INTENT(IN   ) :: IntKiBuf(:)
 ...
 ! buffers to store meshes, if any
  ...
  ELSE
    ...
    END IF
      ...
      END DO
  END IF
 END SUBROUTINE ExtInfw_UnPackOutput
 ...
 SUBROUTINE ExtInfw_C2Fary_CopyOutput( OutputData, ErrStat, ErrMsg, SkipPointers )
    ...
    IF ( .NOT. SkipPointers_local ) THEN
       ...
       END IF
    END IF
 END SUBROUTINE ExtInfw_C2Fary_CopyOutput
 ...
 SUBROUTINE ExtInfw_F2C_CopyOutput( OutputData, ErrStat, ErrMsg, SkipPointers  )
    ...
    IF ( .NOT. SkipPointers_local ) THEN
       ...
       END IF
    END IF
 END SUBROUTINE ExtInfw_F2C_CopyOutput
...
!..................................................................................................................................
 ...
 ELSE 
   ...
   RETURN
 ENDIF 
 END SUBROUTINE ExtInfw_Input_ExtrapInterp
...
IF (ASSOCIATED(u_out%pzVel) .AND. ASSOCIATED(u1%pzVel)) THEN
  ...
  END DO
END IF ! check if allocated
IF (ASSOCIATED(u_out%pxdotVel) .AND. ASSOCIATED(u1%pxdotVel)) THEN
  DO i1 = LBOUND(u_out%pxdotVel,1),UBOUND(u_out%pxdotVel,1)
    b = -(u1%pxdotVel(i1) - u2%pxdotVel(i1))
    u_out%pxdotVel(i1) = u1%pxdotVel(i1) + b * ScaleFactor
  END DO
END IF ! check if allocated
IF (ASSOCIATED(u_out%pydotVel) .AND. ASSOCIATED(u1%pydotVel)) THEN
  DO i1 = LBOUND(u_out%pydotVel,1),UBOUND(u_out%pydotVel,1)
    b = -(u1%pydotVel(i1) - u2%pydotVel(i1))
    u_out%pydotVel(i1) = u1%pydotVel(i1) + b * ScaleFactor
  END DO
END IF ! check if allocated
IF (ASSOCIATED(u_out%pzdotVel) .AND. ASSOCIATED(u1%pzdotVel)) THEN
  DO i1 = LBOUND(u_out%pzdotVel,1),UBOUND(u_out%pzdotVel,1)
    b = -(u1%pzdotVel(i1) - u2%pzdotVel(i1))
    u_out%pzdotVel(i1) = u1%pzdotVel(i1) + b * ScaleFactor
  END DO
END IF ! check if allocated
...
IF (ASSOCIATED(u_out%pzForce) .AND. ASSOCIATED(u1%pzForce)) THEN
  ...
  END DO
END IF ! check if allocated
IF (ASSOCIATED(u_out%pxdotForce) .AND. ASSOCIATED(u1%pxdotForce)) THEN
  DO i1 = LBOUND(u_out%pxdotForce,1),UBOUND(u_out%pxdotForce,1)
    b = -(u1%pxdotForce(i1) - u2%pxdotForce(i1))
    u_out%pxdotForce(i1) = u1%pxdotForce(i1) + b * ScaleFactor
  END DO
END IF ! check if allocated
IF (ASSOCIATED(u_out%pydotForce) .AND. ASSOCIATED(u1%pydotForce)) THEN
  DO i1 = LBOUND(u_out%pydotForce,1),UBOUND(u_out%pydotForce,1)
    b = -(u1%pydotForce(i1) - u2%pydotForce(i1))
    u_out%pydotForce(i1) = u1%pydotForce(i1) + b * ScaleFactor
  END DO
END IF ! check if allocated
IF (ASSOCIATED(u_out%pzdotForce) .AND. ASSOCIATED(u1%pzdotForce)) THEN
  DO i1 = LBOUND(u_out%pzdotForce,1),UBOUND(u_out%pzdotForce,1)
    b = -(u1%pzdotForce(i1) - u2%pzdotForce(i1))
    u_out%pzdotForce(i1) = u1%pzdotForce(i1) + b * ScaleFactor
  END DO
END IF ! check if allocated
...
IF (ASSOCIATED(u_out%forceNodesChord) .AND. ASSOCIATED(u1%forceNodesChord)) THEN
  ...
  END DO
END IF ! check if allocated
 END SUBROUTINE ExtInfw_Input_ExtrapInterp1
...
IF (ASSOCIATED(u_out%pxdotVel) .AND. ASSOCIATED(u1%pxdotVel)) THEN
  DO i1 = LBOUND(u_out%pxdotVel,1),UBOUND(u_out%pxdotVel,1)
    b = (t(3)**2*(u1%pxdotVel(i1) - u2%pxdotVel(i1)) + t(2)**2*(-u1%pxdotVel(i1) + u3%pxdotVel(i1)))* scaleFactor
    c = ( (t(2)-t(3))*u1%pxdotVel(i1) + t(3)*u2%pxdotVel(i1) - t(2)*u3%pxdotVel(i1) ) * scaleFactor
    u_out%pxdotVel(i1) = u1%pxdotVel(i1) + b  + c * t_out
  END DO
END IF ! check if allocated
IF (ASSOCIATED(u_out%pydotVel) .AND. ASSOCIATED(u1%pydotVel)) THEN
  DO i1 = LBOUND(u_out%pydotVel,1),UBOUND(u_out%pydotVel,1)
    b = (t(3)**2*(u1%pydotVel(i1) - u2%pydotVel(i1)) + t(2)**2*(-u1%pydotVel(i1) + u3%pydotVel(i1)))* scaleFactor
    c = ( (t(2)-t(3))*u1%pydotVel(i1) + t(3)*u2%pydotVel(i1) - t(2)*u3%pydotVel(i1) ) * scaleFactor
    u_out%pydotVel(i1) = u1%pydotVel(i1) + b  + c * t_out
  END DO
END IF ! check if allocated
IF (ASSOCIATED(u_out%pzdotVel) .AND. ASSOCIATED(u1%pzdotVel)) THEN
  DO i1 = LBOUND(u_out%pzdotVel,1),UBOUND(u_out%pzdotVel,1)
    b = (t(3)**2*(u1%pzdotVel(i1) - u2%pzdotVel(i1)) + t(2)**2*(-u1%pzdotVel(i1) + u3%pzdotVel(i1)))* scaleFactor
    c = ( (t(2)-t(3))*u1%pzdotVel(i1) + t(3)*u2%pzdotVel(i1) - t(2)*u3%pzdotVel(i1) ) * scaleFactor
    u_out%pzdotVel(i1) = u1%pzdotVel(i1) + b  + c * t_out
  END DO
END IF ! check if allocated
IF (ASSOCIATED(u_out%pxForce) .AND. ASSOCIATED(u1%pxForce)) THEN
  DO i1 = LBOUND(u_out%pxForce,1),UBOUND(u_out%pxForce,1)
...
IF (ASSOCIATED(u_out%pxdotForce) .AND. ASSOCIATED(u1%pxdotForce)) THEN
  DO i1 = LBOUND(u_out%pxdotForce,1),UBOUND(u_out%pxdotForce,1)
    b = (t(3)**2*(u1%pxdotForce(i1) - u2%pxdotForce(i1)) + t(2)**2*(-u1%pxdotForce(i1) + u3%pxdotForce(i1)))* scaleFactor
    c = ( (t(2)-t(3))*u1%pxdotForce(i1) + t(3)*u2%pxdotForce(i1) - t(2)*u3%pxdotForce(i1) ) * scaleFactor
    u_out%pxdotForce(i1) = u1%pxdotForce(i1) + b  + c * t_out
  END DO
END IF ! check if allocated
IF (ASSOCIATED(u_out%pydotForce) .AND. ASSOCIATED(u1%pydotForce)) THEN
  DO i1 = LBOUND(u_out%pydotForce,1),UBOUND(u_out%pydotForce,1)
    b = (t(3)**2*(u1%pydotForce(i1) - u2%pydotForce(i1)) + t(2)**2*(-u1%pydotForce(i1) + u3%pydotForce(i1)))* scaleFactor
    c = ( (t(2)-t(3))*u1%pydotForce(i1) + t(3)*u2%pydotForce(i1) - t(2)*u3%pydotForce(i1) ) * scaleFactor
    u_out%pydotForce(i1) = u1%pydotForce(i1) + b  + c * t_out
  END DO
END IF ! check if allocated
IF (ASSOCIATED(u_out%pzdotForce) .AND. ASSOCIATED(u1%pzdotForce)) THEN
  DO i1 = LBOUND(u_out%pzdotForce,1),UBOUND(u_out%pzdotForce,1)
    b = (t(3)**2*(u1%pzdotForce(i1) - u2%pzdotForce(i1)) + t(2)**2*(-u1%pzdotForce(i1) + u3%pzdotForce(i1)))* scaleFactor
    c = ( (t(2)-t(3))*u1%pzdotForce(i1) + t(3)*u2%pzdotForce(i1) - t(2)*u3%pzdotForce(i1) ) * scaleFactor
    u_out%pzdotForce(i1) = u1%pzdotForce(i1) + b  + c * t_out
  END DO
END IF ! check if allocated
IF (ASSOCIATED(u_out%pOrientation) .AND. ASSOCIATED(u1%pOrientation)) THEN
  DO i1 = LBOUND(u_out%pOrientation,1),UBOUND(u_out%pOrientation,1)
...
IF (ASSOCIATED(u_out%forceNodesChord) .AND. ASSOCIATED(u1%forceNodesChord)) THEN
  ...
  END DO
END IF ! check if allocated
 END SUBROUTINE ExtInfw_Input_ExtrapInterp2
...
!..................................................................................................................................
 ...
 ELSE 
   ...
   RETURN
 ENDIF 
 END SUBROUTINE ExtInfw_Output_ExtrapInterp
...
IF (ALLOCATED(y_out%WriteOutput) .AND. ALLOCATED(y1%WriteOutput)) THEN
  ...
  END DO
END IF ! check if allocated
 END SUBROUTINE ExtInfw_Output_ExtrapInterp1
...
IF (ALLOCATED(y_out%WriteOutput) .AND. ALLOCATED(y1%WriteOutput)) THEN
  ...
  END DO
END IF ! check if allocated
 END SUBROUTINE ExtInfw_Output_ExtrapInterp2
